 ...
 *----------------------------------------------------------------------------*
 *  $Id: MeshLoader.pas,v 1.2 2004/12/11 13:16:23 clootie Exp $
 *----------------------------------------------------------------------------*)
//--------------------------------------------------------------------------------------
...
begin
  ...
  Result:= pMesh.LockAttributeBuffer(0, PByte(pSubset));
  if V_Failed(Result) then Exit;
  ...
  V_(pMesh.ConvertPointRepsToAdjacency(nil, aAdjacency));
  V_(pMesh.OptimizeInplace(D3DXMESHOPT_ATTRSORT or D3DXMESHOPT_VERTEXCACHE, aAdjacency, nil, nil, nil));
...
type
  ...
  public
    ...
    procedure Ignore(Count: Integer; ch: Char);
...
procedure TWifStream.Ignore(Count: Integer; ch: Char);
var
  StartPos: PChar;
begin
  StartPos:= FBufferPos;
...
begin
  ...
  lstrcpynW(m_strMediaDir, wstr, MAX_PATH-1);
  ...
  InitMaterial(pMat^);
  lstrcpynW(pMat.strName, 'default', MAX_PATH-1);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        InitMaterial(pMat^);
        lstrcpynW(pMat.strName, @strName[1], MAX_PATH-1); 
...
begin
  ...
  // Find the file
  Result:= DXUTFindDXSDKMediaFile(strPath, MAX_PATH, strFileName);
  if V_Failed(Result) then Exit;
